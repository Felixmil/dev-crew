...
Inline questions come with two hard constraints worth understanding before you dispatch a fleet. First, an inline question has a fixed timeout (about 60 seconds) after which the model is told there was no answer and to proceed on its own judgment; the timeout is not configurable, and an answer given after it fires does not reach the run. Second, in a background session the prompt may not surface to you at all, and reports describe it either hanging or the model continuing on a default nobody chose. The skill guards against the "continue on a default" failure directly: on any unanswered or timed-out question it stops with the question still recorded in `state.json.pendingQuestion` and the status unchanged, so a re-run re-asks the exact question rather than guessing. It never adopts a default for a question it actually raised.

Given that, the recommendation is firm: **run `manual` and `semi-auto` issues in the foreground**, where you see the question and the 60-second window is a non-issue in practice, and reserve **background dispatch for `auto` runs**, which raise no question at all. A `semi-auto` or `manual` issue can still be run in the background, and the persist-then-stop behavior keeps it correct if a question is missed, but you may have to re-run it once you notice the "Needs input" row, so the foreground is simply less friction.
