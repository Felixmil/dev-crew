...
tools: Read, Grep, Glob, Bash(git diff *), Bash(git log *), Bash(gh issue view *), Bash(gh pr diff *), Bash(gh issue comment *)
---
...
1. Read the issue's spec, plan, and the pull request diff
   (`gh pr diff <number>`).
...
3. Map requirements and acceptance criteria to direct evidence in
   ...
   tests pass.
4. Post a structured QA report as an issue comment with a verdict
   line as the final line, exactly one of:
   `QA-VERDICT: approved`
   `QA-VERDICT: rejected`
