# Implementation plan: file-based issue pipeline (`/issue-pipeline` skill)

## Overview

This plan turns the confirmed spec into an ordered build sequence for a new Claude Code **skill**, `/issue-pipeline`, that drives one GitHub issue through spec -> plan -> build -> QA while keeping the four artifacts, the state machine, and all human interaction on the local filesystem and in the running session (the GitHub issue stays as input, a pull request stays as the ship channel). The skill runs in the session's own main context, so it reads and writes `<repo>.issues/<issue>/` directly and prompts inline via `AskUserQuestion`; it spawns four new file-writing subagents (forked from the existing gh-posting agents, which stay untouched) for the heavy per-phase reasoning, hands them concrete paths, and receives a structured "clarification needed" result instead of scanning a marker out of a comment. State lives in `state.json`, mutated only through a single validated transition step whose edge table is copied verbatim from `odt-transition.sh`. The work is deliberately additive: nothing existing (`openducktor-issue.js`, `odt-transition.sh`, the four agents, `refine-issue`) is modified.
...
5. **A plugin's `skills/` directory is auto-discovered like `agents/` — TRUE.** Installing the plugin exposes the skill in every project with no per-repo copy and no `skills` field in `plugin.json` (the default `skills/` directory is always scanned); the skill is invoked namespaced as `/openducktor-agents:issue-pipeline`. (Source: plugins-reference.) This makes the install one plugin plus a one-file transition-script copy, simpler than the original workflow's two-path install; see the resolved risk and work item 4.
...
| Thing | Name |
| --- | --- |
| Skill (dir + invocation) | dir `skills/issue-pipeline/`; invoked namespaced as `/openducktor-agents:issue-pipeline` (this plan and the spec write it as the shorthand `/issue-pipeline` throughout; both resolve to the same skill) |
| Spec agent | `spec-writer-agent` |
| Plan agent | `plan-writer-agent` |
| Build agent | `build-runner-agent` |
...
Plugin-namespaced agent types (per the existing plugin prefix convention) are `openducktor-agents:spec-writer-agent`, etc. The plugin *package* name (`openducktor-agents`) is a pre-existing identifier and is out of scope to rename; the constraint is about **new** names, and the prefix is how Claude Code namespaces plugin agents. The only place the literal "OpenDucktor"/"odt" text may appear in new files is a lineage comment at the transition table (FR-STATE-1).
...
- **Creates:** `.claude/scripts/issue-state-transition.sh` (in this plugin repo; copied per-target-repo like `odt-transition.sh` is).
- **What:** A bash script `issue-state-transition.sh <issues-root> <issue> <to-status>` that (a) reads the issue's current `status` from `<issues-root>/<issue>/state.json` (treating a missing file/field as `open`), (b) reads the `type:task`/`type:bug` signal *from GitHub* (`gh issue view <issue> --json labels`, read-only, exactly as the original does — FR-STATE-4), (c) checks `current -> to` against an `allowed()` case-statement whose body is **copied character-for-character** from `odt-transition.sh` (same states, same edges, same `is_task` shortcut, same four `*-awaiting-approval` gate edges), and (d) only on success writes the new `status` back into `state.json` (via a `jq` read-modify-write to a temp file, then atomic move), leaving every other field untouched. An illegal transition prints `Transition not allowed: <current> -> <to>` to stderr and exits non-zero **without** mutating the file.
...
- **How to verify:** From a scratch dir, seed `state.json` with `{"status":"open"}`; run the script for a legal edge (`open -> spec-ready`) and confirm `state.json.status` updates and nothing else changes; run an illegal edge (`open -> ai-review`) and confirm non-zero exit, stderr message, and an **unchanged** file. Diff the `allowed()` body against `odt-transition.sh`'s to prove the edge set is identical modulo the prefix strip. Confirm `type:task` seeded on the GitHub test issue enables `open -> in-progress`.
- **Satisfies:** FR-STATE-1, FR-STATE-2, FR-STATE-4 (partial: the read side).

### 2. The four file-writing agents

- **Creates:** `agents/spec-writer-agent.md`, `agents/plan-writer-agent.md`, `agents/build-runner-agent.md`, `agents/qa-review-agent.md`.
...
- **How to verify:** Install the plugin locally (`/plugin marketplace add`, `/plugin install`), confirm the four new `subagent_type`s resolve under the `openducktor-agents:` prefix; unit-invoke `spec-writer-agent` against a throwaway issue with a handed path and confirm it either writes a clean `spec.md` at that path or returns the clarification object (never writes a marker).
...
- **Creates:** `skills/issue-pipeline/SKILL.md`.
...
- **How to verify:** The end-to-end checklist at the end of this plan (acceptance 1-12). Minimal smoke: `/issue-pipeline <n> auto` on a fresh throwaway issue produces all four files + a linked PR and advances status with no issue-thread comments.
...
- **Changes:** `README.md` (add an `/issue-pipeline` section and list the four new agents, the skill, and the new script under "What's here"). No `plugin.json` change is required: `agents/` and `skills/` are both auto-discovered (confirmed against plugins-reference), so no `skills` manifest field is needed.
- **What:** Document the install: the plugin ships the skill **and** the four agents (auto-discovered, invoked as `/openducktor-agents:issue-pipeline`), and only the transition **script** is copied per target repo (scripts are not plugin-discoverable, exactly like `odt-transition.sh`). This is one plugin install plus a one-file script copy, simpler than the original workflow's two-path (copy-workflow-and-script) install. Also document the three modes on two axes, the `<repo>.issues/` layout, `dependsOn`, resumability, the fleet-via-agent-view model, and the execution-mode guidance from the resolved background-session risk (prefer foreground for `manual`/`semi-auto`, background for `auto`, until the background-session reports resolve). Explicitly state the coexistence with the gh-posting pipeline.
...
- **Background-session `AskUserQuestion` reliability — RESOLVED (decision below).** The fleet model and inline-answer flow depend on a background session reliably pausing on `AskUserQuestion` and being answerable from agent view. An issue-tracker search corroborated the spec author's stated concern: there are current, open reports of background/daemon sessions (i) dropping assistant text emitted immediately before an `AskUserQuestion` call, and (ii) not stopping at the question in fully headless/print mode. The design does not depend on those being fixed. **Decision:** proceed exactly as the spec dictates (inline-only, background-based fleet; no fallback answer channel — that stays a Non-goal), with three concrete provisions:
  ...
  2. **`pendingQuestion`-first persistence (code, already required by FR-RESUME-1).** Because the question is persisted before the prompt, a stuck background session is recoverable: the human kills it and re-runs `/issue-pipeline <issue>`, which re-asks the exact question in a fresh session. The resumability mechanism doubles as the mitigation, at no extra cost.
...
1. `/issue-pipeline <n> auto` on a fresh issue -> `spec.md`, `plan.md`, a real linked PR, `build.md`, `qa.md` under `~/Code/<repo>.issues/<n>/`; `state.json.status` walks the full sequence; no question surfaced; zero issue-thread comments and zero PR bookkeeping comments. (acceptance 1)
...
5. Resumability: set `state.json.pendingQuestion`, kill the session before answering, re-run `/issue-pipeline <n>` -> it re-asks that exact question *before anything else*, clears it on answer. (acceptance 5)
...
7. Fleet: dispatch several `/issue-pipeline` background sessions -> separate agent-view rows; a question in one shows "Needs input" and is answerable inline without affecting others; the skill contains no multi-issue launcher. (acceptance 7)
...
11. Bare `/issue-pipeline <n>` (no mode word) runs in semi-auto: surfaces a genuine ambiguity inline but auto-approves artifacts. (acceptance 11)
